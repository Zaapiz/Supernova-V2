;(() => {
  ...
  class o {
    ...
    async fetch(e) {
      ...
      return (
        !!s &&
        new Response(s.body, { headers: s.headers, status: s.status, statusText: s.statusText })
      )
    }
  }
  !('$scramjet' in self) &&
  ...
  async function S(e, t) {
    ...
    if (r.has('url')) return Response.redirect(l(r.get('url'), R(new URL(r.get('url')))))
    try {
      let o = new URL(e.url),
        n = ''
      if (
        ...
        return (
          ...
          new Response(r, { status: i.status, statusText: i.statusText, headers: c })
        )
      }
      let i = new URL(d(o)),
      ...
      return await C(i, n, e.destination, f, this.cookieStore, t, this)
    } catch (r) {
      let t = {
        message: r.message,
        url: e.url,
  ...
  async function C(e, t, r, o, s, n, i) {
    ...
    let c = w(o.rawHeaders, R(e)),
      l = c['set-cookie'] || []
    for (let t in l) n && n.postMessage({ scramjet$type: 'cookie', cookie: t, url: e.href })
    for (let t in (await s.setCookies(l instanceof Array ? l : [l], e), c))
      Array.isArray(c[t]) && (c[t] = c[t][0])
    if ((o.body && (a = await $(o, R(e), r, t, s)), ['document', 'iframe'].includes(r))) {
    ...
    'text/event-stream' === c.accept && (c['content-type'] = 'text/event-stream'),
      ...
      crossOriginIsolated &&
        ['document', 'iframe', 'worker', 'sharedworker', 'style', 'script'].includes(r) &&
        ((c['Cross-Origin-Embedder-Policy'] = 'require-corp'),
        (c['Cross-Origin-Opener-Policy'] = 'same-origin'))
  ...
  class O extends EventTarget {
    ...
    constructor() {
      ...
      ;(e.onsuccess = () => {
        ...
        t.onsuccess = () => {
          t.result && (this.cookieStore.load(t.result), T.log('Loaded cookies from IDB!'))
        }
      }),
